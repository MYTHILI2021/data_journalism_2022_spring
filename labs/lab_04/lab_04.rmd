...
```{r}
# add code here to accomplish the task
maryland_ppp_zcta<- read_csv('data/maryland_zcta.csv') %>% mutate(across(ZCTA5N, as.character))
...
```{r}
# add code here to accomplish the task
maryland_ppp_with_naics_and_demographics <- maryland_ppp_with_naics %>% left_join(maryland_ppp_zcta, by=c("zip5"="ZCTA5N"))
```

## Answer questions

**Q1** What are the top 5 industries (using the NAICS title) with the most PPP applications in Maryland?
**A1** Taxi Service, Beauty Salons, Full-Service Restaurants, Offices of Real Estate Agents and Brokers, All other personal services are the top 5 industries with the most PPP applications.
```{r}
...
**A2** Full-Service Restaurants took 1488 loans with loan amount more than $150,000.
...
**A3** Zip 20850 with at least 20% non-Hispanic Asian population had the largest sum of PPP loans.
...
**A4** Zip 20850 has the largest sum with race 'Asian' and PNHA is 23.5.
...
**A5** Reasons like insufficient data since many loans had unanswered put in the race column.
...
**A6** 20701, 21031,21240 have 13 loans combined with amount $7,253,407.These zip codes cover a very small area.
...
**A7** There are 44 industries with exactly one loan in the data.
