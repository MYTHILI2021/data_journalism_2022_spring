...
**A1.**  Illinois has the most applications and the most undisbursed amounts. There are 54 applications with undisbursed amounts greater than 0.
https://datawrapper.dwcdn.net/Fmcry/3/

```{r}
ppp_barber <- read_csv("data/barber_shops_ppp.csv")

ppp_barber_total <- ppp_barber %>% 
  group_by(state) %>% 
  summarise(
    total = n()
  ) %>% arrange(desc(total))
  
ppp_barber_undisbursed <- ppp_barber %>%
...
ppp_barber_final <- merge( ppp_barber_total, ppp_barber_undisbursed, by = "state") %>% rename( applications=total.x, undisbursed = total.y) %>% arrange(desc(applications))

ppp_barber_topten <- ppp_barber_final %>% head(10)

write_csv(ppp_barber_topten, "data/ppp_barber_topten.csv")
...
**A2.** The new chart, a scatter plot, shows a new metric for ranking the states. By calculating the percentage of undisbursed amounts to their total applications, we get a new result. Los Angeles has the most undisbursed amounts when compared to its total applications received.
I decided to calculate the percentage because there's no standard for calculating which state has the most undisbursed amount which made it harder to compare the states. But by creating percentages, we've standarized the applications and the undisbursed amounts in a single value.
https://datawrapper.dwcdn.net/7xA7L/1/

```{r}
ppp_barber_percent <- ppp_barber_topten %>% 
  mutate(percentage = (undisbursed/applications)*100) %>% 
  arrange(desc(percentage))

write_csv(ppp_barber_percent, "data/ppp_barber_percent.csv")
...
**A3.** Kabbage loans are more frequent in the east and west coasts but not as much in the central part of the country. Even more than the west, Kabbage loans are found to be higher on the east coast. In Florida, the DeSoto county has no data displayed. By looking in the PPP data of DeSoto county, majority of the loan lenders were banks and some entries didn't mention the lender names.About 306 loans were distributed in DeSoto county.

https://datawrapper.dwcdn.net/2gJSc/1/

```{r}
kabbage_counties <- read_csv("data/kabbage_counties.csv")
all_counties <- read_csv("data/all_counties.csv")

counties_final <- merge( kabbage_counties, all_counties, by = c("project_county_name","project_state")) %>% 
  rename(kabbage_total=total.x, total = total.y, match_county = project_county_name, state = project_state)

county_fips <- read_csv("data/county_fips.csv")
counties_final_with_fips <- merge( counties_final, county_fips, by = c("match_county","state"))

counties_percentage <- counties_final_with_fips %>% mutate(percentage = (kabbage_total/total)*100) %>% arrange(desc(percentage))


write_csv(counties_percentage, "data/counties_percentage.csv")
