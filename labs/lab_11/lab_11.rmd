...
**A1.** Ritchie county got the highest total loan amount with 77,752,742.79 amount per 100k. Ritchie got 7653980.00 without population adjusted. There were 9 loans and 9844 population.


```{r}
wv_mining_industry <- wv_ppp_loans %>% filter(str_starts(naics_code, "21"))
wv_mining_final <- wv_mining_industry %>%  group_by(project_county_name) %>% 
                     summarise(
                       total_loans =n(),
                       total_sum =sum(amount))

wv_mining_final <-wv_mining_final %>% rename(COUNTY = project_county_name)
...
wv_loans_per_100k <- wv_county_population %>%
  left_join(wv_mining_final) %>% 
  mutate(total_loans = replace_na(total_loans,0)) %>% 
  mutate(total_sum = replace_na(total_sum,0)) %>% 
...
**A2.** The counties that had the highest loan amounts have mine permits that expire after September, 2021.

```{r}
wv_geodata<- st_read("https://tagis.dep.wv.gov/arcgis/rest/services/WVDEP_enterprise/mining_reclamation/MapServer/0/query?where=1%3D1&outFields=*&geometry=&geometryType=esriGeometryEnvelope&inSR=4326&spatialRel=esriSpatialRelIntersects&outSR=4326&f=json")

wv_mines_2021 <-wv_geodata %>% mutate(expiredate=ymd(expiredate)) %>% 
...
**A3.** Ritchie county which has the highest total loan amount per 100k has no mines.
...
**A4.** HALL DRILLING, LLC accounts for the biggest chunk of the total loan amount.
According to its naics code, HALL DRILLING is part of the Drilling Oil and Gas Wells industry.They provide trucking, drilling,product installation, construction and other roustabout services.
They save 387 jobs, according to the data.
The company could seek PPP loans for paying the workers, for repairs of older equipment, for new equipment, for procuring construction material, etc. 

```{r}
Ritchie_county <- wv_mining_industry %>% 
  group_by( project_county_name ="RITCHIE", name, jobs_retained) %>% 
  arrange (desc(amount))
