...
```{r}
us_doj_url<-"https://www.justice.gov/criminal-fraud/cares-act-fraud"
...
A2. There are 31 judicial districts.

```{r}


judicial_districts <- us_doj_url %>% 
  read_html() %>% 
  html_elements('p b i') %>% 
  html_text() %>% 
  as_tibble()
...
A4. Diamond Blue Smith is a rapper, going by alias "Baby Blue", in the hip hop group Pretty Ricky. Smith was charged with wire fraud, bank fraud, attempt and conspiracy to commit wire fraud and bank fraud in the criminal complaint.Smith obtained massive PPP loans for his two companies, Throwbackjersey.com LLC, and Blue Star Records LLC, using falsified documents.
I would've assigned a reporter to write a story about this case when the complaint was filed because it's newsworthy. Smith, who is a celebrity and has a net worth of several millions, doesn't qualify for a PPP loan which were aimed to help small businesses during the pandemic. He's a famous personality and he tried to fraud the system. People need to know who's gatekeeping and misusing their money.
...
A5.32.9% of all judicial districts have filed cases related to PPP fraud.
...
Active_courts <- wiki_courts %>%
                 read_html() %>% 
                 html_table()
Active_courts <- Active_courts [[3]]

Nonactive_courts <- Active_courts %>% anti_join(judicial_districts, by=c("Region"="value"))
 
percentage_court=((31/94)*100)
...
A6. The source data that the DOJ might be working from to determine whether there's fraud within the application might be insufficient and so far, they might have been able to verify only these cases. Furthermore, these are cases that are charged only up to December 6, 2021, according to the website.
...
state_list<-state_url %>% read_html() %>% 
                            html_table()
state_list<-state_list[[1]]
state_final_list<-tibble()
for (row_number in 1:nrow(state_list)) {
  ...
  state_final_list<-state_final_list %>% 
