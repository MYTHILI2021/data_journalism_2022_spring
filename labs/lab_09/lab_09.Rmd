...
ggplot() + 
  ...
  theme(
    ...
    x = "Date",
    y = "Total Loan Count",
    caption = "Source: SBA PPP loan database for West Virginia"
...
state_total <- get_acs (geography = 'state',
...
state_total_loans <- state_ppp_all %>%
  inner_join(state_total, by=c( "state" = "NAME"))%>%
...
state_total_loans %>%
  ...
  coord_flip()+
  labs(
    title ="Top 10 PPP states per 100k",
    x = "State",
    y = "Loans per 100k",
    caption = "source: SBA PPP loan database for West Virginia"
  )
...
**A3.**  Compared to the people who's loan amount was not forgiven, there was a lot of people who had their loans forgiven. We can even say that most people had their loans forgiven and very few didn't have their loans forgiven. These few had a high PPP loan amount. The scatterplot also shows that some loans were partially forgiven.
