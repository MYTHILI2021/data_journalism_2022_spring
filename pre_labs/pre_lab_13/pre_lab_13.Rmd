...
**Answer** The dataframe outputs data on state's electoral data including votes, percentage of votes, and percentage of businesses which received PPP loans in that state.
```{r}
...
**Answer** The result shows a scatter plot of percentage of Trump states receiving PPP loans, increasing from left to right.
```{r}
...
### Task 4: Plot data
**Task** Run the following code. Describe the plot. What do you see?
**Answer** Compared to the above chart, this result shows a chart that has the dots scattered father away from each other and not following a certain direction too closely. 
...
**Answer** Percent of businesses receiving PPP loans is more closely related to Trump votes than individual PPP amount per individual.
```{r}
reveal_data %>%
...
**Answer** The output shows some significant values such p-value and t-value and how the correlation value was calculated.Low p-values indicate that there's less chance the variable has correlation equal to zero which means low chance of businesses are not correlated to the votes Trump received. There's some connection there.
```{r}
...
**Task** Run the following code. Describe the model output. What do you see?
**Answer** The output shows a higher p-value which means there's a higher chance the PPP amount per employee is unrelated to the votes Trump received. A lower correlation value also indicates the same.
```{r}
...
**Answer** Clinton states had lower ppp amount per employee on average than Trump states. Trump states had a higher percentage of businesses receiving PPP loans than Clinton states.
```{r}
reveal_data %>%
  group_by(vote_2016) %>%
  summarise(
...
**Answer** Low t-values mean that there's not a significant difference between the Clinton and Trump groups with respect to PPP amount per employee.
```{r}
...
### Task 10: Run t-test
**Task** Run the following code. Describe the output. What do you see?
**Answer** Higher t-values mean that there's a similarity between the Clinton and Trump groups with respect to percent of businesses receiving PPP loans.
```{r}
