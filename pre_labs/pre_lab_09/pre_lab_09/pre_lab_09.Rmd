...
**Answer** It has 195865 rows and 63 columns.
...
**Answer** The code below demonstrates the top ten counties in Maryland PPP loans data with highest amount of loans per each county. To do this, first we grouped by the county names and then summarised the data with total loans per each county. Then we arranged them in descending order. Using head(10) we displayed the top ten rows of the resulted data.
...
**Answer** ggplot() function presents with an empty canvas to build our chart on.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** geom_bar() creates a bar chart and aes or aesthetics are for the x and y axes. The code also uses + instead of %>%. We have the x-axis as project_county_name and weight or y-axis as total_loans.
...
**Answer** The reorder() function allows the variables to be arranged in an ascending order. It helps the viewers to understand the chart easier than before.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The coord_flip() function switches the x and y axes, resulting in a more suitable chart for the given data. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** The theme_minimal() function gives an overall better theme and visibility such as a white background compared to the grey in the previous chart or the mid-number lines.
...
**Answer** Installed ggthemes package.
...
**Answer** We used the economist theme from the ggthemes. It gives the chart a background color and there's contrast in colors which increases visibility and makes it visually interesting.
```{r}
maryland_ppp_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(project_county_name,total_loans), weight=total_loans)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** Using the labs() function meaning labels, we gave the chart a title, caption, and named the x and y axes differently.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** The loans are summarized according to their approved dates. For example, the first result has 28 loans approved on April 3.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Using the geom_line() function, we can create a line chart. Once again, the aesthetics are x and y, i.e, the x and y axes. The resultant line chart shows the number of loans approved over a range of dates, essentially a dateline.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** Using the scale_x_date() function, we can set date breaks and labels. In the resultant line chart, the dates are divided by months. For the labels, "%b" is the abbreviated month and "%y" is the two-digit year.
...
**Task** Run the following code. Explain what you see after running the code in the space below.  
**Answer** The date labels are styled at an angle to make them more readable.
