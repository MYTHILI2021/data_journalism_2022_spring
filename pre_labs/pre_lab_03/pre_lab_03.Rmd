...
**Answer** "amount" <dbl> -- double, name" <chr> -- character, "id" <dbl> -- double
...
**Answer**: 192959 loans had NA as their franchise_name.275 loans had Subway as their franchise_name.
...
**Answer**: 135073 loans had the forgiven_amount as NA. 
...
**Answer**: 135073 loans had the forgiveness_date as NA. It's the same number as the forgiveness_number. 
...
**Answer**: 135073 loans had forgiveness_date and forgiveness_amount as NA. It is the same number as the codeblocks above. We can conclude that these loans were not forgiven and their forgiven dates don't exist either. The data tallies against each other. 
...
**Answer** There were no loans in the data for October 2020. There exists a gap between August 2020 and January 2021.
```{r}
ppp_maryland_loans %>%
...
**Answer** 10 million is the largest loan amount.6 is the smallest loan amount.Both the loan amounts are suspicious, especially the smaller amount because how could $6 help a business run itself.
```{r}
...
**Answer** Yes, all fields with amounts align.
...
**Answer** There are 6 rows in the data set.
...
**Answer** <chr> -- character data type
```{r}
...
**Answer** The amount column is stored as a character, not a number. No calculations would work on the column.
The first row is exactly the same as the second.Redundant data doesn't give us accurate results.
The city field has five different forms or misspellings of Arnold
...
**Answer** Header names cannot start with numbers so clean_names() function added an "x" before "1_id".
...
**Answer** The error says the following:Error: Problem with `summarise()` column `total_amount`.
i `total_amount = sum(amount)`.
x invalid 'type' (character) of argument
Since the datatype of amount is character type, R is not able to perform any calculations with the amounts. R views the amount values as a number with no meaningful value.
...
**Answer** The new datatype for amount is <dbl> or double.
...
**Answer** 2033179	is the total loan amount.
```{r}
# cleaning function
total_arnold_md_loans <- cleaned_arnold_md_loans %>%
  summarise(total_amount = sum(amount))
...
**Answer** Two records of one business. The business is called Stuart Jones LLC.
...
**Answer** The new dataframe has only 5 rows.
...
**Answer** Two mispellings of Arnold -- "Arnld" and "Anold" remain.
...
**Answer** One mispelling remains -- "Arnld"
...
**Answer** Zero mispellings remain.
```{r}
# cleaning function
cleaned_arnold_md_loans <- arnold_md_loans %>%
  clean_names() %>%
