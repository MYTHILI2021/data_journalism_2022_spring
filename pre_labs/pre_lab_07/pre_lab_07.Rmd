...
**Answer** The web page has the table "NAICS sectors." It has two simple columns of two-digit NAICS codes -- some are described in ranges -- and the broad category of sector they belong to.
...
**Answer** The source code for the tables exist on the page. Each entry is listed for each row of the table.
...
**Answer** R assigned a variable to the link with the url as the value.
...
**Answer** The naics_industry variable shows code for the html document.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** The "Inspect" feature allowed us to see the source code for the specific elements of the page such as the table 2. The element contained tags to create the table such as <tr>, <td>, etc.
...
**Answer** R scraped the tables found on the url and changed them into data frames using the html_table() statement. R scraped a list of 6 data frames from the url.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** It had all the 6 data frames and their data, datatypes listed as tibbles.
...
**Answer** The data frame is one of tables extracted from the url.
...
**Task** Follow the directions in the previous paragraph. Briefly describe what you see there.
**Answer** The data frame is the second table that's extracted from the url. It has the sectors and their descriptions.
...
**Answer** The table we want from the link is now converted into a data frame. We assigned the variable 'naics_industry' to the data frame and displayed it.
...
**Answer** The last record, row number 21, is removed using slice(-21). By doing so, only the required data is in the data frame and therefore, cleaned.
...
**Answer** The outputis similar to the last output in the previous chapter. We assign a variable to the url, read the url and then extract all the tables from the url.Then, we select the table we require and clean it accordingly and view. The data frame naics_industry has 20 rows and 2 columns.
```{r}
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** The link above provides numbers for employees in different naics sectors.
...
**Task** Run the following code to read in the html. Briefly describe the output that appears below the codeblock.
**Answer** The output just displays some html code for the url we read into an R variable.
...
**Answer** Instead of extracting all tables from a url/html document, we're using html_element to extract the specific table from a link. To do so, we need an "xpath" or table id which we can find by viewing the source code of a website.Each element of the page has a unique ID to help distinguish it. We can then use it to extract the element or table we require from the url.
The output displays the specific table we scraped of from the website but it is not in the form of a useable table or dataframe yet.
...
**Answer** We're converting the html code for a table to an R dataframe with the function html_table().
...
**Answer** The output is the selection of some columns of row 2 from the html table employment_info.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** The website is pointing to the employment and unemployment table.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** The website is pointing to the dynamic changes in employment table.
...
**Answer** The output printed all ten print statements.
```{r}
...
**Answer** No output is displayed under the codeblock but a variable "list_of_sectors" is created in the environment table with an array of 10 entries.
```{r}
...
**Answer** The list of sectors is printed by using 1-2 lines of code involving a for loop.
```{r}
...
**Answer** The output said that a "sector_list" object doesn't exist.
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** The list of sectors is printed even if we changed the name in the for loop (from sector to x).
```{r}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Task** Run the following code to use a for loop to print list of sectors. Briefly describe the output that appears below the codeblock.
**Answer** The code didn't work because both the for condition and the statement inside for loop didn't have matching names. The object "sector_name" wasn't found.
```{r, error=TRUE}
# Define list of sectors
list_of_sectors <- c("Agriculture, Forestry, Fishing and Hunting", "Mining, Quarrying, and Oil and Gas Extraction", "Utilities", "Construction", "Manufacturing",
"Wholesale Trade", "Retail Trade", "Transportation and Warehousing", "Information", "Finance and Insurance")
...
**Answer** The code printed numbers from 1 to 10.
```{r}
# For loop that includes instructions that refer to a variable that doesn't exist.
for (number in 1:10) {
  print(number)
...
**Answer** The output displayed all the records of the naics_industry data frame.
...
**Answer** The output displays the number of rows in naics_industry which is 20.
...
**Answer** The output displays all the row numbers in naics_industry using the for loop.
...
**Answer** The output shows employment info for November 2021 from the employment_info table.
...
**Task** Run the following code to load table from last chapter. Briefly describe the output that appears below the codeblock.
**Answer** The output displays the naics_industry table.
```{r}
# Define url of page we want to scrape
...
**Answer** We added another column to naics_industry, "sector_url", with the urls as values. The urls have the sector numbers listed out.
...
**Answer** we added another column called sector_xpath_id with the xpath ids for respective sectors.
...
**Answer** Since we filtered out the Public Administration record, the output now has 19 rows.
...
**Answer** Each row is isolated from the other rows in the naics_industry data frame.
...
**Answer** Each row with its row number, url and xpath id is printed.
```{r}
...
**Answer** 19 dataframes are displayed in the output contained inside a nested list.
...
**Answer**  19 dataframes are extracted from the nested list with cleaned column names.
...
**Answer** 19 dataframes with 10 columns each, containing the sector code and sector description.
...
**Task** Run the following for loop to clean up tables. Briefly describe the output that appears below the codeblock.
**Answer** Cleaned versions of the 19 dataframes are displayed in the output.
...
**Answer** The output table is result of binding employment info to the naics_industry table.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It's the employee statistics for Mining, Quarrying, and Oil and Gas Extraction.
...
**Task** Visit the web page linked above. Briefly describe what you see there.
**Answer** It's the employee statistics for the Agriculture, Forestry, Fishing and Hunting industry.
...
**Answer** The suspicious value of "5.4" in the agriculture row is removed.
